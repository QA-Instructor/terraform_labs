resource "azurerm_storage_account" "lab3bsa-web" {
  name                     = var.sa_name_web
...
# create a section for the the index.html upload

resource "azurerm_storage_blob" "indexupload" {
  name                   = "index.html"
  storage_account_name   = azurerm_storage_account.lab3bsa-web.name
